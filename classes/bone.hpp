...
public:
	...
	{
		uintptr_t player_model = driver.Read<uintptr_t>(player + 0x128); // BaseEntity->model
		uintptr_t bone_transforms = driver.Read<uintptr_t>(player_model + 0x48); // Model->boneTransforms
		uintptr_t entity_bone = driver.Read<uintptr_t>(bone_transforms + (0x20 + (bone_index * 0x8)));
